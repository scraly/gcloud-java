...
public class PublisherSmokeTest {
  ...
  public static void executeNoCatch(String projectId) throws Exception {
    PubSub pubSub = PubSubOptions.getDefaultInstance().toBuilder().setProjectId(projectId)
        .build().getService();

    String topicId = "dep-debug-topic-" + System.currentTimeMillis();
    Topic topic = pubSub.create(TopicInfo.of(topicId));
    Message message = Message.of("Hi there!");
